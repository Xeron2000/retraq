...
vi.mock('../components/PositionDetails', () => ({
  default: ({ trade }: { trade?: { id?: number } | null }) => <div data-testid='position-details' data-trade-id={trade?.id ?? ''} />,
}));
...
vi.mock('lightweight-charts', () => {
  ...
  const createTimeScale = () => ({
    ...
    getVisibleRange: vi.fn(() => ({ from: Math.floor(routeSeed.entryTime / 1000), to: Math.floor(routeSeed.exitTime! / 1000) })),
  ...
  return {
    ...
    createSeriesMarkers: createSeriesMarkersSpy.mockImplementation(() => ({ setMarkers: markerSetSpy })),
    createChart: vi.fn(() => ({
...
describe('Phase 2 replay flow smoke', () => {
  beforeEach(() => {
  ...
  it('boots replay from the route seed, keeps markers, and persists stepped progress', async () => {
    ...
    expect(screen.getByTestId('position-details')).toHaveAttribute('data-trade-id', String(routeSeed.tradeId));
    ...
    await waitFor(() => {
      const raw = window.localStorage.getItem(buildReplayProgressStorageKey(routeSeed.tradeId));
      const parsed = raw ? JSON.parse(raw) : null;
      expect(parsed?.cursorTime).toBeGreaterThan(routeSeed.entryTime);
    });
  });

  it('restores saved replay progress when entering replay without a route seed', async () => {
    ...
    window.localStorage.setItem(
      ...
      serializeReplayProgress(
        createReplayProgressRecord({
          tradeId: routeSeed.tradeId,
          cursorTime: routeSeed.entryTime + 1_800_000,
          speed: 4,
          savedAt: 1_712_900_000_000,
        }),
      ),
    ...
    expect(await screen.findByText(new Date(routeSeed.entryTime + 1_800_000).toLocaleString('zh-CN'))).toBeInTheDocument();
